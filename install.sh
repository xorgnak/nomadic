...
DISTRO_GEMS='pry sinatra redis-objects cinch gmail sinatra-pubsub eventmachine rb-inotify mqtt tw rake sinatra-websocket'
...
EOF
    ...
    echo "usage: ./install.sh -> install nomadic configuration files locally."
    echo "usage: sudo ./install.sh --server -> install nomadic packages."
    echo "usage: sudo ./install.sh --pretty -> install nomadic packages and desktop."
else
    cat <<EOF > /etc/issue
  ##############################
 ######### NOMADIC LINUX ########
